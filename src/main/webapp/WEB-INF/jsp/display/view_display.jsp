...
<body data-help-page="view_display">
					...
					<div class="col-md-12">
						...
						<c:choose>
							...
							<c:when test="${not empty courtSiteList}">
								...
								<form:form commandName="displaySearchCommand" action="${context}/display/view_display" method="POST" class="form-horizontal">

								<%-- CSRF Guard token where uri equals form action --%>
								<%--<input type="hidden" name="<csrf:tokenname/>" value="<csrf:tokenvalue uri="${context}/display/view_display"/>"/>--%>
								...
								<spring:hasBindErrors name="displaySearchCommand">
								...
								<div class="form-group">
									<spring:bind path="xhibitCourtSiteId">
										<div class="col-md-12 ${requestScope['org.springframework.validation.BindingResult.displaySearchCommand'].hasFieldErrors('xhibitCourtSiteId') ? 'has-error' : ''}">
									...
									<div class="col-md-12">
										<security:authorize access="hasRole('ROLE_ADMIN')"> 
											<button id="btnAmend" class="btn btn-primary" name="btnAmend" value="amend">
												<span class="glyphicon glyphicon-edit"></span> Amend Display</button>
											<button id="btnAdd" class="btn btn-primary" name="btnAdd" value="add">
												<span class="glyphicon glyphicon-plus"></span> Create Display</button>
											<button id="btnDelete" class="btn btn-primary" name="btnDelete" value="delete">
												<span class="glyphicon glyphicon-minus"></span> Delete Display</button>
										</security:authorize>
									</div>
								</div>
							</form:form>
							...
							<%-- No court site data found --%>
								<div class="alert alert-danger">
									<p>No Court Sites present in database so unable to View/delete Displays.</p>
								</div>
							</c:otherwise>
						</c:choose>
					</div>
