...
<head>
    ...
    <title>Manage Hearing Type</title>

    <%@ include file="../common/stylesheets.jsp"%>

</head>

<body data-help-page="view_hearing">
					...
					<div class="col-md-12">
						...
						<c:choose>
							...
							<c:when test="${not empty courtSiteList}">
								...
								<form:form commandName="hearingTypeSearchCommand" action="${context}/hearing/view_hearing" method="POST" class="form-horizontal">

								<%-- CSRF Guard token where uri equals form action --%>
								<%--<input type="hidden" name="<csrf:tokenname/>" value="<csrf:tokenvalue uri="${context}/hearing/view_hearing"/>"/>--%>
								...
								<spring:hasBindErrors name="hearingTypeSearchCommand">
								...
								<div class="form-group">
									<spring:bind path="xhibitCourtSiteId">
										<div class="col-md-12 ${requestScope['org.springframework.validation.BindingResult.hearingTypeSearchCommand'].hasFieldErrors('xhibitCourtSiteId') ? 'has-error' : ''}">
							...
							<%-- No court site data found --%>
								<div class="alert alert-danger">
									<p>No Court Sites present in database so unable to View/Amend Hearing Types.</p>
								</div>
							</c:otherwise>
						</c:choose>
					</div>
	...
	<%-- Hearing Specific javascript files --%>
